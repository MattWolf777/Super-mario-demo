import pygame
from support import import_cut_graphics


class Tile(pygame.sprite.Sprite):
    def __init__(self, size, x, y):
        super().__init__()
        self.image = pygame.Surface((size, size))
        self.rect = self.image.get_rect(topleft=(x, y))

    def update(self, shift):
        self.rect.x += shift


class StaticTile(Tile):
    def __init__(self, size, x, y, surface):
        super().__init__(size, x, y)
        self.image = surface


class Background(StaticTile):
    def __init__(self, path, size, x, y):
        super().__init__(size, x, y, pygame.image.load(path).convert_alpha())
        offset_x = x + size
        offset_y = y + size
        self.rect = self.image.get_rect(bottomleft=(offset_x, offset_y))


class AnimatedTile(Tile):
    def __init__(self, size, x, y, sprite_row_number, path):
        super().__init__(size, x, y)
        self.frames = import_cut_graphics(path)
        self.sprite_row_index = sprite_row_number
        self.frame_index = (sprite_row_number - 1) * 4
        self.image = self.frames[self.frame_index]

    def animate(self):
        self.frame_index += 0.08
        if self.frame_index > self.sprite_row_index * 4:
            self.frame_index -= 4
        self.image = self.frames[int(self.frame_index)]

    def update(self, shift):
        self.animate()
        self.rect.x += shift
